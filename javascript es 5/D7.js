...
*/
concatenaStringhe("Emanuele","Gardina");

function concatenaStringhe(str1, str2) {
  let s1 = str1.slice(0,2);
  let s2 = str2.slice(-3); // oppure str2.slice(str2.length -3)
  let result = (s1 + s2).toUpperCase();
  console.log(result);
...
*/
function arrPari(params) {
  let nomeVariabile = params.filter(num => num % 2 === 0);
  return nomeVariabile
}
console.log(arrPari(ra))
...
function arrStrFunc(strArr) {
  return strArr.map(ele => ele.length )
}
